...
public class ThreadClass extends Thread {
	//the run method when called by start() checks whether the given number is prime or not.
	public void run()
	{
	try
		{
		 Scanner sc=new Scanner(System.in);  
		   ...
		   System.out.println(" Thread1:Enter the number to check");  
		...
		//number.Bcos divisor of a number cannot be more than of half of itself.
		   for(int i=2;i<=inputNumber/2;i++)
				...
				if (inputNumber %i == 0){ //checks whether the number is divisible by anything other than 1 and itself.
					System.out.println("Thread1:Given number " +inputNumber + " is not a prime");
				return;//returns if the number is prime.
				}

		}
		   System.out.println("Thread1:Given number " +inputNumber + " is  a prime");
	}
	catch(Exception e){}
	}
	}
